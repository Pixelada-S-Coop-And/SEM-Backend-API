...
class Affiliate extends CI_Controller {
    ...
    /*
        This API method opens a affiliate session by passing it the correct email and password as parameters
        Parameters: token, email, pass
    */
    
    private function login($data){ 
        if(empty($data->pass))
            $this->response_error('Debes introducir la contraseña');

        //Buscamos el usuario en la Base de datos
        $affiliate = (object) $this->affiliate_model->affiliate_by_pass($data->pass);
        
        //Si no se encuentra el usuario
        if(empty($affiliate->email))
            $this->response_error('La contraseña es incorrecta');

        if($this->affiliate_model->affiliate_has_session($affiliate->id)){
            $this->response_error('Ya ha abierto sesión en otro dispositivo');
        }

        $data = $this->affiliate_model->login_affiliate($affiliate->email);
    ...
    /*
        This API method closes a affiliate session by passing a session id as parameters
        Parameters: token, session_id
    */
    public function logout($data){  
        if(!empty($data->session_id))
            $this->affiliate_model->logout($data->session_id);
    ...
    /*
        This API method creates a affiliate account
    ...
    */
    private function register($data){
        ...
        if(!$this->affiliate_model->email_validation($data->email)){
            $this->response_error('El email introducido no es válido');
            return false;
        }

        if($this->affiliate_model->affiliate_exists($data->email)){
        ...
        $this->affiliate_model->new_affiliate($data);

        $affiliate = (object) $this->affiliate_model->affiliate($data->email);
        $token = base64_encode(json_encode(array('time' => time(), 'email' => $affiliate->email, 'pass' => $affiliate->pass)));
        ...
        $this->send_email($affiliate->email, 'SEM - Activar cuenta de usuario',$message);

        $this->response_ok('Usuario registrado correctamente', $data);
    }

    /*
        This API method edits the data of a affiliate with the session open
    ...
    */
    public function edit($data){  
        ...
        if($this->affiliate_model->affiliate_exists_edit($affiliate_id, $data->email))
            $this->response_error('El email introducido ya fue registrado por otro usuario');
  
        if(!empty($data->pass)&&strlen($data->pass)<6)
            $this->response_error('La contraseña debe tener al menos 6 caracteres');

        $this->affiliate_model->edit_affiliate($affiliate_id, $data);

        $this->response_ok('Usuario editado correctamente', $data);
    }

    /*
        This API method returns the data of a affiliate with the session open
        Parameters: token, session_id
    */
    public function data($data){  
        ...
        $this->response_ok('Datos de usuario', $affiliate);
    }
    ...
    */
    public function form_send($data){  
        if(empty($data->session_id)||!$this->affiliate_model->session_exists($data->session_id))
            $this->response_error('Usuario no logueado');
        ...
        $message = '<p>';
        $message .= '<strong>Nombre</strong>: '.$affiliate->name.' '.$affiliate->surnames.'<br />';
        if(!empty($affiliate->id_card)) $message .= '<strong>DNI</strong>: '.$affiliate->id_card.'<br />';
        if(!empty($affiliate->section_name)) $message .= '<strong>Sección sindical</strong>: '.$affiliate->section_name.'<br />';
        if(!empty($affiliate->job)) $message .= '<strong>Trabajo</strong>: '.$affiliate->job.'<br />';
        if(!empty($affiliate->job_position)) $message .= '<strong>Puesto de trabajo</strong>: '.$affiliate->job_position.'<br />';
        if(!empty($affiliate->birthdate)) $message .= '<strong>Fecha de nacimiento</strong>: '.date('d/m/Y', strtotime($affiliate->birthdate)).'<br />';
        if(!empty($affiliate->email)) $message .= '<strong>Email</strong>: '.$affiliate->email.'<br />';
        if(!empty($affiliate->phone)) $message .= '<strong>Teléfono</strong>: <a href="tel:34'.str_replace(' ', '', $affiliate->phone).'">'.$affiliate->phone.'</a><br />';
        if(!empty($affiliate->address)) $message .= '<strong>Dirección</strong>: '.$affiliate->address.'<br />';
        if(!empty($affiliate->zipcode)) $message .= '<strong>Código Postal</strong>: '.$affiliate->zipcode.'<br />';
        if(!empty($affiliate->location)) $message .= '<strong>Municipio</strong>: '.$affiliate->location.'<br />';
        if(!empty($affiliate->province)) $message .= '<strong>Provincia</strong>: '.$affiliate->province.'<br />';
        $message .= '</p><br /><hr /><br />';

        $message .= '<h3>Consulta del afiliado</h3>';
        if(!empty($data->subject)) $message .= '<p><strong>Asunto</strong>: '.$data->subject.'</p>';
        $message .= $data->content;
        $message .= '<br />';
        $affiliate_msg = '<p>Su mensaje se ha enviado correctamente.</p>';
        //$message .= '<br /><hr /><br /><pre>'.print_r($affiliate, true).'</pre>'; 
        $to = $this->option_model->option('form_email', '');
        $this->send_email($to, 'Consulta de afiliado a SEM - '.$data->subject, $message, 'Consulta a Autonomía Sur', 'Sindicato de Empleados Municipales');
        $this->send_email($affiliate->email, 'Consulta de afiliado a SEM - Su mensaje se ha enviado',  $affiliate_msg.$message, 'Consulta a Autonomía Sur', 'Sindicato de Empleados Municipales');
    ...
    /*
        This API method resets a affiliate's password by passing an email as a parameter
    ...
    */    
    private function resetpass($data){
        ...
        $this->send_email($affiliate->email, 'Autismo Códoba - Recuperar contraseña', $message);
    ...
    */ 
    ...
    private function send_email($to, $subject, $message, $title='', $subtitle='', $cc=''){
    ...
    public function form_send_post(){  
        $this->form_send($this->post_data());
    }

    public function form_send_get(){  
        $this->form_send($this->get_data());
    }
