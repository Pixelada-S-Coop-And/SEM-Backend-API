...
class Advice extends CI_Controller {
    ...
    /*
        This API method returns a list of all advices in the database 
    ...
    */
    private function list($data){
        ...
        $advices = $this->advice_model->advices();

        $this->response_ok('Listado de consejos', $advices);
    }

    /*
        This API method returns the data of a specific advice
        Parameters: token, session_id, advice_id
    */
    private function item($data){
         if(empty($data->advice_id)||empty($data->session_id)||!$this->user_model->session_exists($data->session_id))
            $this->response_error('Usuario no logueado');
        
        $advice = $this->advice_model->advice(intval($data->advice_id));

        $this->response_ok('Contenido de consejo', $advice);
    }
